  ...
  Editar Serviço
        ...
        <form action="{{ route('servicos.update', $servico) }}" method="POST"
            ...
            <div>
                ...
                <select name="barbearia_id"
                    ...
                    @foreach($barbearias as $b)
                        <option value="{{ $b->id }}" @if($servico->barbearia_id == $b->id) selected @endif>
                            {{ $b->nome }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <textarea name="descricao" rows="3"
                          class="w-full bg-[#241b16]/60 border border-yellow-500/20 rounded-lg px-4 py-2
                                 text-[#f5e6d3] focus:ring-1 focus:ring-yellow-500/30">{{ old('descricao', $servico->descricao) }}</textarea>
            </div>

            {{-- PREÇO --}}
            <div>
                <label class="block text-sm text-yellow-300/80 mb-1">Preço (R$)</label>
                <input type="number" step="0.01" name="preco" value="{{ old('preco', $servico->preco) }}"
            ...
            <div>
                ...
                <input type="number" name="duracao_minutos" value="{{ old('duracao_minutos', $servico->duracao_minutos) }}"
                       class="no-spinner w-full bg-[#241b16]/60 border border-yellow-500/20 rounded-lg px-4 py-2 text-[#f5e6d3]
                              focus:ring-1 focus:ring-yellow-500/30"
                       required>
            </div>
