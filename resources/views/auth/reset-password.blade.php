@extends('layouts.auth')

@section('title', 'Redefinir Senha')

@section('content')
<div class="min-h-screen flex items-center justify-center bg-[#1a1410]">
    <div class="w-full max-w-md bg-[#1a1410]/70 backdrop-blur-xl border border-yellow-500/20 rounded-2xl p-8 shadow-2xl">

        <h2 class="text-3xl font-bold text-yellow-300 mb-6 text-center drop-shadow-lg tracking-wide">
            Redefinir Senha
        </h2>

        <p class="text-yellow-200/70 text-sm mb-6 text-center leading-relaxed">
            Insira sua nova senha abaixo para concluir a redefinição.
        </p>

        <!-- Status -->
        @if (session('status'))
            <div class="mb-4 p-3 bg-green-600/20 text-green-300 border border-green-500/30 rounded text-sm">
                {{ session('status') }}
            </div>
        @endif

        <!-- Erros -->
        @if ($errors->any())
            <div class="mb-4 p-3 bg-red-600/20 text-red-300 border border-red-500/30 rounded text-sm">
                @foreach ($errors->all() as $error)
                    <p>{{ $error }}</p>
                @endforeach
            </div>
        @endif

        <form method="POST" action="{{ route('password.store') }}" class="space-y-5">
            @csrf

            <input type="hidden" name="token" value="{{ $request->route('token') }}">

            <!-- Email -->
            <div>
                <label class="block text-yellow-200/80 text-sm mb-2">Email</label>
                <input id="email" name="email" type="email" value="{{ old('email', $request->email) }}" required readonly
                       class="w-full px-4 py-2 bg-[#241b16]/60 text-gray-400 border border-yellow-500/20 rounded-lg cursor-not-allowed">
            </div>

            <!-- Nova Senha -->
            <div>
                <label class="block text-yellow-200/80 text-sm mb-2">Nova Senha</label>
                <input id="password" name="password" type="password" required
                       class="w-full px-4 py-2 bg-[#241b16]/60 text-white placeholder-gray-400 border border-yellow-500/20 rounded-lg focus:ring-2 focus:ring-yellow-400/50 focus:outline-none">
            </div>

            <!-- Confirmar Senha -->
            <div>
                <label class="block text-yellow-200/80 text-sm mb-2">Confirmar Senha</label>
                <input id="password_confirmation" name="password_confirmation" type="password" required
                       class="w-full px-4 py-2 bg-[#241b16]/60 text-white placeholder-gray-400 border border-yellow-500/20 rounded-lg focus:ring-2 focus:ring-yellow-400/50 focus:outline-none">
            </div>

            <button type="submit"
                class="w-full py-2 rounded-lg font-semibold text-[#1a1410] bg-gradient-to-r from-yellow-600 via-yellow-500 to-yellow-400 hover:brightness-110 shadow-lg hover:shadow-xl transition duration-200">
                Redefinir Senha
            </button>
        </form>

        <div class="mt-6 text-center">
            <a href="{{ route('login') }}" class="text-yellow-300/80 hover:text-yellow-400 hover:underline">
                Voltar ao login
            </a>
        </div>
    </div>
</div>
@endsection
